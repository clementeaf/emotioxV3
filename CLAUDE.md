...
- **Participant rendering** (v0.45.0): `ScreenerRenderer` (choice question, reutiliza `ChoiceQuestion`), `ImplicitAssociationRenderer` (motor IAT completo: instrucciones → priming → trials → feedback, teclado E/I + touch, 3 tipos de test), `EyeTrackingRenderer` (click/tap tracking como proxy, countdown timer, resolución S3). Los 3 producen el formato exacto que esperan los endpoints de analytics.
...
- `participant-frontend/src/components/renderers/ImplicitAssociationRenderer.tsx` — IAT: motor de trials completo (instrucciones → priming → trial → feedback → complete). Teclado E/I + botones touch. Response `component_id = 'iat-trials'`.
