...
- **Eye Tracking híbrido** (v0.41.1): ruta `/eye-tracking-hybrid` en participant-frontend. Flujo: intro → instrucción → calibración 9 puntos (desktop) → estímulo silencioso 10s → heatmap de zonas 3x3. Desktop usa BlazeGaze (`webeyetrack` npm, calibración instantánea via `handleClick`). Tablet/móvil usa taps como proxy. Resultado = mapa de calor por zonas (no punto exacto). Pipelines legacy: `/eye-tracking-test` (MediaPipe ridge) y `/labs/eye-tracking` (research-frontend BlazeGaze lab).
