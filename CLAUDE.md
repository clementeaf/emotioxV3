...
- **Eye Tracking híbrido** (v0.42.1): ruta `/eye-tracking-hybrid` en participant-frontend. Flujo: intro → instrucción → calibración 9 puntos (desktop) → estímulo silencioso 10s → heatmap de zonas **4×4 (16 zonas)**. Desktop usa BlazeGaze (`webeyetrack` npm) con calibración explícita (`adapt()` multi-frame, fire-and-forget). Tablet/móvil usa taps como proxy. Zonas se mapean relativas al bounding rect de la imagen (no a la ventana). Pipeline legacy `/eye-tracking-test` eliminado; solo queda `/labs/eye-tracking` (research-frontend BlazeGaze lab).
