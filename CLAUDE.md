...
- **Video per-frame render (v0.85.2)**: `VIDEO_SAMPLE_INTERVAL` env var controls DINO keyframe spacing. Local `0.0` = every frame (fluid). cPanel `2.0` = keyframes only. `VIDEO_MAX_DIM` controls downscale limit. `isLegacyAttentionStimulus` fixed for DINO video with AI analysis.
