...
- **Eye Tracking híbrido** (v0.42.1): ruta `/eye-tracking-hybrid` en participant-frontend. Flujo: intro → instrucción → calibración **4 puntos** (centro de cada cuadrante, desktop) → estímulo → heatmap **2×2 (4 cuadrantes)**. Desktop usa BlazeGaze (`webeyetrack` npm) con calibración explícita y campo IDW sobre residuos. Tablet/móvil usa taps como proxy. Zonas se mapean relativas al bounding rect de la imagen (no a la ventana). Pipeline legacy `/eye-tracking-test` eliminado; solo queda `/labs/eye-tracking` (research-frontend BlazeGaze lab). **v0.42.2:** afinado `hybridZoneGrid` (menos capas redundantes), constraints de cámara compartidos (ideal 720p), `BlazeGazeFrameStats` con resolución de captura y aviso suave si el frame es bajo; `docs/eye-tracking-heatmap.md` actualizado.
