...
- **Condicionalidad study question fix** (v0.57.0): `studyModulesWithOptions` detecta componentes por `type` (`radio`, `checkbox-list`, `option-list`) además de `isChoice`/`choice-`. Parsea JSON de values para labels legibles.
- **Condicionalidad "Link with module"** (v0.57.0): tercer tipo de condición (`LinkedModuleConditionality`). Encadena visibilidad entre módulos — "mostrar si el módulo enlazado también se muestra". `linkableModules` lista todos los módulos del estudio (excepto Welcome/ThankYou/ResearchConfig) con numeración ordinal global. `useNavigation` evalúa recursivamente con protección anti-loop.
- **Orden global cross-stage** (v0.57.0): `studyModulesWithOptions` y `linkableModules` usan `stage.order_index × 10000 + mod.order_index` para comparación correcta entre stages. `globalOrderIndex` prop en SmartVOCModuleCard y CognitiveTaskModuleCard.
- **Numeración de preguntas en builder** (v0.57.0): `questionNumber` prop en cards, alineado con dropdown de conditionality y página de resultados.
- **Screening option padding fix** (v0.57.0): `ScreenerQuestionDrawer` ya no hace pad a 3 opciones al editar — muestra exactamente las guardadas.
