...
- **Video thermal heatmap (v0.84.0)**: IDW per-pixel interpolation + colormap LUT reemplaza simpleheat para videos. `renderThermalImageData` (downsample 8x, 6-pass box blur, gamma 2.0, bilinear upsample). `VideoThermalGrid` inline en `AttentionPredictionCard` — cachea thermal + grid overlays, `requestAnimationFrame` loop para playback en tiempo real. Canvas alineado al contenido visual del video (descuenta letterbox bars). Divider ajustable controla visibilidad del heatmap via `clipPath`. 11 tests.
- **Video temporal modulation (v0.84.1)**: Base thermal (IDW acumulado) se construye una vez. Per-frame: `buildModulatedThermal` clona `ImageData` y multiplica alpha por proximidad a hotspots (4px step). `resolveFrameIndex` trackea `timeupdate`/`seeked`. Tres capas: `buildBaseThermal` (once) → `buildModulatedThermal` (per frame) → `buildGridOverlay` (per frame). `compositeFrame` solo dibuja 3 imágenes. 22 tests.
- **Warm heatmap gradient restored (v0.84.2)**: `HeatmapRenderer` gradients revertidos de azul→púrpura a verde→amarillo→naranja→rojo (estilo original pre-v0.80.0). Aplica a imágenes en Attention Prediction. Videos usan rendering thermal separado (`VideoThermalGrid`).
