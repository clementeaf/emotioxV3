...
- **CSV column selector (v0.73.4)**: When a CSV/Excel with multiple columns is uploaded, `detectCsvColumns` detects headers + preview rows. `CsvColumnSelector` component shows interactive table — user picks which column to analyze. `parseDocument(file, columnIndex)` passes the selection. Applied in `CreateResearchForm` drawer and `InsightsFindingView`. Single-column files skip the selector. Column count derived from max row width (handles sparse XLSX arrays).
- **Insights Finding custom prompt (v0.73.4)**: Collapsible "Analysis Prompt" panel in `InsightsFindingView`. Default prompt = neuromarketing/FMCG specialist. Researcher can edit per-research, saved to `config.insightsPrompt`. Backend `analyzeInsights(entries, fileName, customPrompt?)` uses custom if set, default otherwise. "Custom" badge when modified.
