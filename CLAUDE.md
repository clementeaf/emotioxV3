...
- **Video temporal modulation (v0.84.1)**: Heatmap varía por frame. Acumulado como base espacial; `modulateAccumulatedByFrame` aplica boost/atenuación por proximidad a hotspots del frame activo (Gaussian falloff, base 25%). `resolveFrameIndex` trackea `timeupdate` del `<video>`. Caches se reconstruyen solo al cambiar de frame. 22 tests.
