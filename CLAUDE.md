...
- **Saliency rendering** (v0.51.1): `HeatmapRenderer` usa dual renderer. Datos densos de saliencia → colormap directo pixel a pixel (enfoque OGAMA/OpenCV) con LUT 256 colores, alpha cuadrático (`val² × 140`), threshold 0.4, overlay 45%. Datos sparse de clicks → simpleheat (sin cambios). Referencia: OGAMA (Gaussian kernel + palette LUT), iLab Neuromorphic Vision Toolkit (Itti, USC).
