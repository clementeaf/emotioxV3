...
- **Attention Prediction** (v0.51.0): research type sin stages/módulos. El investigador sube imágenes (stimuli) desde un Drawer al crear el research. Las imágenes se almacenan vía `mediaService` (mismo flujo que Navigation Flow / Preference Test: `generateUploadUrl` → `fetch PUT` → `saveMetadata` → `getMediaUrl`). Las referencias se guardan en `researches.config` como `{ stimuli: [{ url, mediaId, name }] }`. El builder muestra `AttentionPredictionView` con `NavigationTestCard` para visualizar heatmaps de predicción. Sidebar muestra stimuli en vez de stages. Ruta: `/research/:id/builder/stimulus/:stimulusId`. Tipo en BD: `"Attention's Prediction"` (con apóstrofe).
