...
- **Condicionalidad study question fix** (v0.57.0): `studyModulesWithOptions` ahora detecta componentes de opción por `type` (`radio`, `checkbox-list`, `option-list`) además de `isChoice`/`choice-`. Modules reales de BD no tenían `isChoice` — ahora aparecen en el dropdown.
