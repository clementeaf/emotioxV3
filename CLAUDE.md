...
- **Executive Summary (v0.71.0)**: Prompt in Spanish. Gathers all module types (SmartVOC, Screener, Cognitive, Ranking, IAT, verbatims). Demographics as context only. `POST /executive-summary` accepts `participantIds` for completion filtering. Cached in `config.executiveSummary`.
