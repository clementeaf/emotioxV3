...
- **Video Attention Prediction (v0.76.0)**: Client extracts frames every 2s (Canvas API, `extractVideoFrames.ts`, max 60, CORS-safe blob download). `POST /video-predict` runs `predictAttentionFast` (single-pass, no TTA) per frame, SSE progress via UUID jobId (no token auth). Results: `stimulus.heatmapData` (accumulated), `stimulus.frames[]` (per-frame), `stimulus.temporalGrid[]` (4x4). Single persistent `<video>` element across tabs (no remount). AOI Editor hidden for video. Image tabs use `display:none` instead of unmount. Video container: `calc(100vh-250px)` height, `max-w/h-full` centered.
