...
/**
 * Seeds research techniques into the database
 */
const seedResearchTechniques = async (): Promise<void> => {
    const connection = await pool.getConnection();
    try {
        console.log('Starting seed for research techniques...');
        ...
        for (const technique of techniques) {
            ...
            const [checkRows] = await connection.query<Array<{ id: string }>>(
                'SELECT id FROM research_techniques WHERE name = ?',
                [technique.name]
            );
            ...
            if (checkRows.length === 0) {
                ...
                valueParts.push('?', '?');
                paramValues.push(technique.name, technique.description);
                ...
                const query = `INSERT INTO research_techniques (${columns.join(', ')}) VALUES (${valueParts.join(', ')})`;
...
export { seedResearchTechniques };

// Run if executed directly
if (require.main === module) {
    seedResearchTechniques().catch((error) => {
